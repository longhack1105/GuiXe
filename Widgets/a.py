...
class XeRa:
    def __init__(self, window):
        ...
        if Global.test == 1:
            ...
            childTab.setSize(leftPos=20, rightPos=620, topPos=20, bottomPos=520)#("400x370+100+100")
        ...
        ###khai báo
        hoTen = StringVar()
        self.width_pic, self.height_pic = 30, 30
        #self.check=0
        ...
        cboDayInHH = ttk.Combobox(TimeIn)
        cboDayInHH.pack(side=LEFT)
        cboDayInMM = ttk.Combobox(TimeIn)
        ...
        cboDayOutHH = ttk.Combobox(TimeOut)
        cboDayOutHH.pack(side=LEFT)
        cboDayOutMM = ttk.Combobox(TimeOut)
        ...
        etrCountDay = Entry(frm_countday)
    ...
    def resize_pic(self, event):
        ...
        try:
            self.etrPlateOut.config(width=self.display4.winfo_width()-1000)
            self.etrPlateIn.config(width=self.display4.winfo_width()-1000)
